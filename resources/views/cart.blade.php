...
@section("content")
    ...
    <main class="pt-90">
        ...
        <form class="position-relative bg-body" method="POST" action="{{route('cart.coupon.remove')}}">
            @csrf     
            @method('DELETE')                   
            <input class="form-control text-success fw-bold" type="text" name="coupon_code" placeholder="Coupon Code" value="{{ Session::has('coupon') ? Session::get('coupon')['code'] . ' Applied!' : '' }}"
>
